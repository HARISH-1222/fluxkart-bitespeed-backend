...
export const getContactList = async (req: Request, res: Response) => {
  try {
    ...
    const isNewCustomer: boolean = await contactModel.isNewCustomer(
      requestData.email,
      requestData.phoneNumber.toString()
    );

    if (isNewCustomer) {
      candidateDetails = await contactModel.createNewCustomer(
        requestData.email,
        requestData.phoneNumber.toString()
      );
    } else {
      const isEmailContains: boolean =
        await contactModel.isEmailOfCustomerExist(requestData.email);
      const isPhoneNumberContains: boolean =
        await contactModel.isPhoneNumberOfCustomerExist(
          requestData.phoneNumber.toString()
        );

      if (!(isEmailContains && isPhoneNumberContains)) {
        //Fist task
        await contactModel.createNewCustomer(
          requestData.email,
          requestData.phoneNumber.toString()
        );
      }
      ...
      //second task
      candidateDetails = await contactModel.checkIsPhoneNumOrEmailMatch(
        requestData.email,
        requestData.phoneNumber.toString()
      );
    }
